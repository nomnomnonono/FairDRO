...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x, get_inter=False, reid=False):
        ...
        b1 = self.dropout(self.layer1(h))
        b2 = self.dropout(self.layer2(b1))
        b3 = self.dropout(self.layer3(b2))
        b4 = self.dropout(self.layer4(b3))
        """
        b1 = self.layer1(h)
        b2 = self.layer2(b1)
        b3 = self.layer3(b2)
...
def resnet12(pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet("resnet12", BasicBlock, [1, 1, 2, 1], pretrained, progress, **kwargs)
